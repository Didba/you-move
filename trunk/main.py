...
class FindMyBusiness(webapp.RequestHandler):
  def get(self):
    my_position_lat = 41.442726
    my_position_lon = 12.392578
    ...
    all_trips = models.Trip.queryall()
    logoutUrl = ' '
    loginUrl = ' '
    if users.get_current_user():
      logoutUrl = users.create_logout_url(str(next_url))
    else:
      loginUrl = users.create_login_url(str(next_url))
    template_values = {'trip_list': all_trips,
                       'logoutUrl': logoutUrl,
                       'loginUrl': loginUrl ,
                       'user': users.get_current_user()}
    
    path = os.path.join(os.path.dirname(__file__), os.path.join('templates', 'my_directions.html'))
  ...
  def post(self):
    ...
    my_stores = models.Store.queryall(lat=user_pos_lat, lon=user_pos_lon)
...
class AddTrip(webapp.RequestHandler):
  ...
  def post(self):
    ...
    logging.info(name)
    addressfrom = self.request.get('addressfrom')
    addressto = self.request.get('addressto')
    waypoints = self.request.get('waypoints')
    new_trip = models.Trip.add(name=name, waypoints=waypoints)
    all_trips = models.Trip.queryall()
    logoutUrl = ""
    loginUrl =" "
    if users.get_current_user():
...
class AddBusiness(webapp.RequestHandler):
  ...
  def post(self):
    ...
    models.Store.queryall(self)
    self.response.out.write('done')

def main():
  application = webapp.WSGIApplication([('/map/add_biz', AddBusiness),
                                        ('/map/add_trip', AddTrip),
                                        ('/map', FindMyBusiness) ],
                                       debug=True)
